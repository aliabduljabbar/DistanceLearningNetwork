...
- (void) loadContent {
    ...
    if (!_label || !_slider) {
        ...
        _label.text = @"Slide to get Eride";
        ...
        sliderFrame.size.width -= 8; //each "edge" of the track is 2 pixels wide
        _slider.frame = sliderFrame;
        _slider.center = ctr;
        _slider.backgroundColor = [UIColor clearColor];
        UIImage *thumbImage = [UIImage imageNamed:@"Car"];//[self thumbWithColor:[UIColor colorWithRed:200.0/255.0 green:200.0/255.0 blue:200.0/255.0 alpha:1.0]];
...
- (void)layoutSubviews {
    ...
    _label.frame = CGRectMake(sliderWidth+5,
                              CGRectGetMidY(self.bounds) - (labelSize.height / 2.0),
                              CGRectGetWidth(self.bounds) - sliderWidth,
                              labelSize.height
                              );
